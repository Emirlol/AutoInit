...
class InitProcessor(private val environment: SymbolProcessorEnvironment) : SymbolProcessor {
	...
	override fun process(resolver: Resolver): List<KSAnnotated> {
		environment.logger.info("Trying to generate $INIT_FILE_NAME.kt")
		...
		for (symbol in symbols) {
			if (symbol is KSClassDeclaration) {
				if (symbol.classKind == ClassKind.OBJECT) {
					...
					else environment.logger.error("Only public objects can be annotated with @AutoInit", symbol)
				} else environment.logger.error("Only objects can be annotated with @AutoInit", symbol)
			} else environment.logger.error("Only singleton classes (objects) can be annotated with @AutoInit", symbol)
		}

		environment.logger.info("Found ${classes.size} classes to initialize.")
	...
	override fun finish() {
		when {
			ksFile == null -> {
				environment.logger.error("No file to write to.")
				return
			}

			classes.isEmpty() -> {
				environment.logger.warn("No classes to initialize.") // Could be a normal case, so it's a warning.
				return
			}
		}

		classes.sortBy { it.annotations.first { it.shortName.asString() == AutoInit::class.simpleName || it.shortName.asString() == AutoInit::class.qualifiedName }.arguments.first().value as Int }
		...
		environment.codeGenerator
			...
			.use {
				it.append(
// This has no indentations, because with indents we have to add way more \t to balance it out when we .trimIndent
					syntaxHighlighter(
						"""
package ${ksFile!!.packageName.asString()}

/**
 * This file is generated by the InitProcessor to initialize features marked with [AutoInit][me.rime.symbols.init.AutoInit]. Do not edit it, as it will be overwritten.
 *
...
"""
		...
		environment.logger.info("Generated $INIT_FILE_NAME.kt")
	}
	...
	companion object {
		...
		/**
		 * A simple syntax highlighter for Kotlin code, utilizing the IntelliJ IDEA's language injection feature.
		 *
		 * I don't know why this can't be applied to string literals directly, so this is a workaround.
		 */
		fun syntaxHighlighter(@Language("kotlin") code: String) = code.trimIndent()
	}
}